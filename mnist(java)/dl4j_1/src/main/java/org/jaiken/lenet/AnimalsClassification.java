...
//        MultiLayerNetwork network = new AlexNet(height, width, channels, numLabels, seed, iterations).init();
		...
		/**
		 * Data Setup -> define how to load data into net: - recordReader = the reader
		 * that loads and converts image data pass in inputSplit to initialize -
		 * dataIter = a generator that only loads one batch at a time into memory to
		 * save memory - trainIter = uses MultipleEpochsIterator to ensure model runs
		 * through the data for all epochs
		 **/
